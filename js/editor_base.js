...
function initLights() {
  ...
  state.lighting.forEach(ld => {
    ...
    if (ld.type === 'ambient') {
      ...
      threeJsLights[ld.id] = light;
      scene.add(light);
      scene.add(light.target); // Asegurar que el target esté en la escena para que el Helper calcule bien la matriz

      // Crear Helper
      const helper = new THREE.DirectionalLightHelper(light, 1.0); // Tamaño más grande
      ...
      const sphereGeom = new THREE.SphereGeometry(0.4, 16, 16); // Esfera más grande
